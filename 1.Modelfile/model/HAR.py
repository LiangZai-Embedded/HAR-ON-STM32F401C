...
def segment_signal(data, window_size = 90):
    ...
    for (start, end) in windows(data['timestamp'],window_size):
        ...
        if(len(data['timestamp'][start:end])==window_size):
            ...
            #
            #
            labels = np.append(labels,stats.mode(data['activity'][start:end])[0][0])  # most common value
        else:
            print("exception")
...
# # # # # # # # #   reading the data   # # # # # # # # # #
# Path of file #
# dataset = readData(r'actitracker_raw.csv')
# print(dataset.iloc[:,dataset.columns=='x-axis'])



# plotting a subset of the data to visualize
# for activity in np.unique(dataset['activity']):
#     subset = dataset[dataset['activity']==activity][:400] # ==Downstairs/Jogging/Sitting/Standing/Upstairs/Walking
...
# segments, labels = segment_signal(dataset) #segments (24141, 90, 3) 24141个样本90X3图片
...
# segments.shape[0]->24141
numOfRows = segments.shape[1]    # 90
numOfColumns = segments.shape[2] #  3
numChannels = 1
numFilters = 128 # 128 number of filters in Conv2D layer
...
dropOutRatio = 0.2
# reshaping the data for network input  reshapedSegments ：(24141, 90, 3, 1)
reshapedSegments = segments.reshape(segments.shape[0], numOfRows, numOfColumns,numChannels)
...
def cnnModel():
    model = Sequential()
    # adding the first convolutionial layer with 128 filters and 2 by 2 kernal size, using the rectifier as the activation function
    ...
    model.add(Flatten())
    # adding first fully connected layer with 64 outputs
    model.add(Dense(numNueronsFCL1, activation='relu'))
    #adding second fully connected layer 64 outputs
...
model = cnnModel()
model.fit(trainX,trainY, validation_split=1-trainSplitRatio,epochs=Epochs,batch_size=batchSize,verbose=1)
