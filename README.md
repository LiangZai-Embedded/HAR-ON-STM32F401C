# HAR-ON-STM32F401C
## 概述
* 在STM32F401C-DISCO开发板上轻量级的卷积神经网络(CNN)，进行人体活动识别(HAR)，识别姿态包括步行、慢跑、上楼、下楼、站姿、坐姿。开发板正面朝下放置在右前裤兜中，系统板载LSM303DLHC加速度计，通过读取三轴加速度数据和模型处理进行用户活动的识别。 
...
* 最终部署在STM32F401C-DISCO开发板上C-Model模型是原模型文件`model.h5`导入`X-CUBE-AI`插件后，选择Keras,8倍压缩率后生成的。
